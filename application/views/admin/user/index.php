    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex mb-4">
                ...
                <a href="<?= base_url('user/tambah') ?>" class="btn btn-sm btn-success ms-auto">Tambah</a>
            </div>
            <div class="card">
                        ...
                        <tbody>
                            ...
                            foreach ($user as $row) : ?>
                                <tr>
                                    ...
                                    <td class="text-start"><?= $row->role == 1 ? 'Admin' : 'Pengajar' ?></td>
                                    <td class="text-center">
                                        <a href="<?= base_url('user/edit/' . $row->id_user) ?>" class="btn btn-sm btn-warning">Edit</a>
                                        <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#hapus" data-bs-id="<?= $row->id_user ?>" data-bs-data="<?= $row->username ?>">hapus</button>
                                    </td>
                                </tr>
                            <?php endforeach ?>
                        </tbody>
...
<script>
    ...
    if (exampleModal) {
        exampleModal.addEventListener('show.bs.modal', event => {
            ...
            link.href = `<?= base_url('user/hapus/') ?>${id}`
        })
    }
</script>